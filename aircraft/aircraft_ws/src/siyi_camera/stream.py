...
class UDPSender:
    '''
    Streams image frames to a local UDP port (5600) as H.264 RTP,
    mimicking the GStreamer command line behavior.
    '''
    def __init__(self, host='127.0.0.1', port=5600, debug=False) -> None:
        self._host = host
        self._port = port
        self._fps = 15 # Should match or be lower than the SIYIRTSP frame rate
        self._width = 640
        self._height = 480
        self._frame = None
        self._stopped = False
        self._st_thread = threading.Thread(target=self.loop, daemon=True)

        self._logger = logging.getLogger(self.__class__.__name__)
        LOG_FORMAT=' [%(levelname)s] %(asctime)s [UDPSender::%(funcName)s] :\t%(message)s'
        logging.basicConfig(format=LOG_FORMAT, level=logging.DEBUG if debug else logging.INFO)
        
        # GStreamer Command to encode and stream raw BGR frames to UDP
        # rawvideo -> videoconvert (format conversion) -> x264enc (encode to H.264) -> rtph264pay (wrap in RTP) -> udpsink (send over UDP)
        # Note: We must encode here since the YOLO node expects H.264 RTP on port 5600.
        self._gst_command = [
            'gst-launch-1.0',
            # Source: Raw BGR input from Python pipe
            'appsrc', 'is-live=true', 'format=time', 'caps=video/x-raw,format=BGR,width={},height={},framerate={}/1'.format(self._width, self._height, self._fps), '!',
            # Processing and Encoding
            'videoconvert', '!',
            'x264enc', 'tune=zerolatency', 'byte-stream=true', '!',
            'rtph264pay', 'config-interval=1', 'pt=96', '!',
            # Sink: Send to UDP
            'udpsink', f'host={self._host}', f'port={self._port}', 'sync=false', 'async=false'
        ]
        
        self._p = None # Subprocess Popen object

    def setImageSize(self, w=640, h=480):
        self._width = w
        self._height = h
        # Note: If changing size/fps after init, you must reconstruct self._gst_command
    ...
    def start(self):
        self._logger.info("Starting UDP Sender pipe to %s:%d", self._host, self._port)
        try:
            # 启动 GStreamer 管道
            self._p = subprocess.Popen(self._gst_command, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            self._st_thread.start()
        except FileNotFoundError:
            self._logger.error("Error: 'gst-launch-1.0' command not found. GStreamer is required.")
            sys.exit(1)
        except Exception as e:
            self._logger.error("Could not create GStreamer pipeline. Error %s", e)
            sys.exit(1)

    def stop(self):
        self._logger.warning("UDP streaming is stopped.")
        self._stopped = True
        if self._p:
            self._p.terminate()
            try:
    ...
    def sendFrame(self) -> bool:
        ...
        try:
            # 确保帧大小匹配管道预期
            if self._frame.shape[0] != self._height or self._frame.shape[1] != self._width:
                frame_to_send = cv2.resize(self._frame, (self._width, self._height), interpolation=cv2.INTER_AREA)
            else:
                frame_to_send = self._frame

            # 将帧的原始字节写入管道
            self._p.stdin.write(frame_to_send.tobytes())
            return True
        except Exception as e:
            self._logger.error("Error in sending frame to UDP pipe: %s", e)
            return False

    def loop(self):
        while(not self._stopped):
            duration = 1.0 / self._fps
            start_t = time()
        ...
        self._logger.warning("UDP streaming loop is done")
        return        
